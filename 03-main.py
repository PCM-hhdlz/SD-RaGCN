...
from imports.utils import train_val_test_split,train_val_test_split_uniform
from sklearn.metrics import classification_report, confusion_matrix

torch.manual_seed(123)      # 为 PyTorch 库设置随机数生成器的种子

EPS = 1e-10

os.environ["CUDA_VISIBLE_DEVICES"] = "1"
print("GPU situation:",torch.cuda.is_available())
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

parser = argparse.ArgumentParser()
parser.add_argument('--epoch', type=int, default=0, help='starting epoch')
parser.add_argument('--n_epochs', type=int, default=100, help='number of epochs of training')
parser.add_argument('--batchSize', type=int, default= 320, help='size of the batches')
parser.add_argument('--dataroot', type=str, default='/root/Experiments/SD_RaGCN/SD_RaGCN_pytorch/data/ABIDE_pcp/dparsf/filt_global/process_data_data_augmentation/', help='root directory of the dataset')
...
fold = opt.fold
writer = SummaryWriter(os.path.join('./log',str(fold)))    # 将每fold训练的日志文件放到指定目录中

################## Define Dataloader ##################################

dataset = ABIDEDataset(path,name)
dataset.data.y = dataset.data.y.squeeze()
dataset.data.x_static[dataset.data.x_static == float('inf')] = 0
dataset.data.x_dynamic[dataset.data.x_dynamic == float('inf')] = 0

# tr_index,val_index,te_index = train_val_test_split(fold=fold)
tr_index,val_index,te_index = train_val_test_split_uniform(fold = fold)
...
def train(epoch):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        output, w1, w2, s1, s2 = model(data.x_static, data.edge_index_static, data.batch, data.edge_attr_static, data.pos_static)
